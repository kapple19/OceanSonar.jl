...
```
c = ocean_celerity(:munk |> Val, 1e3, 1e2; ϵ = 1e-3)
```
"""
...
"""
```
c::Float64 = ocean_celerity(:munk |> Val, x::Real, z::Real; ϵ = 7.37e-3)
```
...
"""
```
c::Float64 = ocean_celerity(:index_squared |> Val, x::Real, z::Real; c₀ = 1550.0)
```
...
@parse_models_and_arguments ocean_celerity
...
(cel::OceanCelerity)(
    x::Real, z::Real; kwargs...
) = ocean_celerity(cel.model, x, z; kwargs...)

OceanCelerity() = OceanCelerity(Symbol() |> Val)

list_models(::Type{OceanCelerity}) = list_models(ocean_celerity)
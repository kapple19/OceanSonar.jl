public KEEPTOKENS
public PASCAL_REGEXES
public AbstractTextCase
...
AbstractTextCase(TC::AbstractTextCase) = TC()
...
function AbstractTextCase(textcase::Symbol)
	...
	textcases = subtypes(AbstractTextCase{T} where {T}) .|> String .|> Symbol
    @assert textcase in textcases """
    $textcase not an implemented `subtype` of `AbstractTextCase`.
    Consider extending as a custom type, e.g.:
    ```julia
	...
	Then either
	```
    OceanSonar.textcasesetup(::$textcase) = (sep = "-^-v-", cap = $cap)
    ```
	or
	```
	function OceanSonar.textcase(::$textcase, text::AbstractString)
		text = OceanSonar.textcase(::OtherCase, text)
	end
	```
    """

	TC = subtypes(AbstractTextCase{textcase}) |> only
	return TC()
end

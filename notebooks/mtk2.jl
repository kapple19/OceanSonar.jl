...
# ╔═╡ 9439f2ed-1dc5-4333-934c-9e49a61e9915
using Pkg

# ╔═╡ c3ef64c0-03d8-11ef-38c5-035a60e72ba5
using ModelingToolkit

# ╔═╡ 018a6d5d-f7c9-4cba-a2e7-bc6d8dd6e24c
using OrdinaryDiffEq

# ╔═╡ cf19a33d-f6b8-4db5-aa58-eed8b305c437
using Plots

# ╔═╡ 27930d05-f61d-4681-95a5-4cc3e3048476
Pkg.status()

# ╔═╡ 88fe0db6-58cb-4fe4-897f-20cb4bc91914
@mtkmodel Eikonal begin
	...
	@continuous_events begin
		[x ~ x_max] => ((t1, t2, t3, t4) -> terminate!(t1), [], [], [])
		[z ~ 0] => [ζ ~ -ζ]
	end
...
# ╔═╡ 9987b4d8-9c1a-4906-8c16-b492e6e57e28
function munk_profile(x, z; ϵ = 7.37e-3)
	z̃ = 2(z - 1300)/1300
	1500(1 + ϵ * (z̃ - 1 + exp(-z̃)))
end

# ╔═╡ 6123e783-bb7f-4719-a1b7-24e7cf1826d6
c = munk_profile

# ╔═╡ a0ccd3e8-91ca-4ee2-9717-a5540561ad46
@mtkbuild eik = Eikonal(c = c, x_max = 10e3)

# ╔═╡ f167af9b-3c38-4695-acdc-5a1643fcb549
z_src = 1e3

# ╔═╡ cdf6730f-a8de-43d5-83ef-7c57c87ae4ea
angles = [-1.1, -1, 0, 1, 1.1] * acos(c(0.0, z_src) / c(0.0, 0.0))

# ╔═╡ d8f898a0-3f8c-4438-b868-6688d86c7328
init_cond(θ₀) = [
	0.0; z_src;
	[cos(θ₀), sin(θ₀)] / c(0.0, z_src)
]

# ╔═╡ 848f3025-a1fa-432c-8c1d-e569d93b6323
prob = ODEProblem(eik, init_cond(0), (0, 200e3))

# ╔═╡ 9c372062-4f73-43a3-9018-c4d21f7d12dc
sol = solve(prob, Tsit5())

# ╔═╡ 6b5a9aa0-e31e-4af2-b16e-f6aca6e15350
plot(sol, idxs = (:x, :z), yflip = true)

# ╔═╡ d1cff8c1-8443-404a-825e-38d2ef06122f
function prob_func(prob, i, repeat)
	remake(prob, u0 = init_cond(π * (rand() - 0.5)))
end

# ╔═╡ 94a1ac29-1fbb-47f3-93d9-c5ca0b6a75ae
ens_prob = EnsembleProblem(prob, prob_func = prob_func)

# ╔═╡ 87673d46-b120-40fa-b048-9ada54ea7768
sim = solve(ens_prob, Tsit5(), EnsembleThreads(), trajectories = 5)

# ╔═╡ 3b40d052-2a41-4e28-8537-928ebb93d55c
plot(sim, idxs = (:x, :z), yflip = true)
...
manifest_format = "2.0"
project_hash = "e9560e45a5c81ce4c0bc1ff3aeb93c7c4c152196"
...
git-tree-sha1 = "43f984aae6f3d2bee9832f14da78eb965f6765e8"
...
git-tree-sha1 = "816176bca8a93f8f50a33853e0933d6c4ec116d0"
...
git-tree-sha1 = "e2a3d8dfaffce3d3b8b5a0967c0cfcc89d34e425"

...
# ╔═╡ 5dcca020-7428-11ef-0196-91b62f198522
using CairoMakie

# ╔═╡ 9e58207e-9435-4a51-8cea-62e22e21e27d
using AlgebraOfGraphics

# ╔═╡ 3d968336-b357-42b9-9202-43b358d1c0a0
using AlgebraOfGraphics: density

# ╔═╡ 68208619-2b6a-4ed8-952a-f740529e59d9
using DataFrames

# ╔═╡ 4404ea3d-2e07-493f-aa2c-a7248c51bd32
using LIBSVM

# ╔═╡ f9cffdd0-d8a9-4228-a8ee-341f4c690e21
using PalmerPenguins

# ╔═╡ 736d535d-efa3-41ce-b12b-cc0d2aa1fe2a
penguins = dropmissing(DataFrame(PalmerPenguins.load()))

# ╔═╡ 92c247fb-ba18-4c13-92ce-ccf0dedc84b2
penguin_frequency = frequency() * data(penguins) * mapping(:species => "Species")

# ╔═╡ 4bb38cf3-35ca-4a06-9f85-5efde225eda0
axis = (width = 225, height = 225)

# ╔═╡ a55731e0-5b64-4bde-972d-c776558ae6c5
draw(
	penguin_frequency;
	axis = axis
)

# ╔═╡ 5c0eff77-e821-4e5d-86d6-8559faa3a05d
draw(
	penguin_frequency * mapping(color = :island);
	axis = axis
)

# ╔═╡ e7142507-a0fc-42cf-9267-81a554b14772
draw(
	penguin_frequency * mapping(color = :island, dodge = :island);
	axis = axis
)

# ╔═╡ 3db68fd3-0da6-4e57-8396-17df10249db9
draw(
	penguin_frequency * mapping(color = :island, stack = :island);
	axis = axis
)

# ╔═╡ aeb39d77-81dc-4ad4-a842-188634d7273f
draw(
	data(penguins) * mapping(:bill_length_mm, :bill_depth_mm);
	axis = axis
)

# ╔═╡ d7825a37-354a-4a65-933d-708a1711f803
penguin_bill = data(penguins) * mapping(
    :bill_length_mm => (t -> t / 10) => "Bill Length (cm)",
    :bill_depth_mm => (t -> t / 10) => "Bill Depth (cm)",
)

# ╔═╡ 5102a605-f12e-450c-a164-8b22f1f6208c
draw(penguin_bill; axis = axis)

# ╔═╡ 7cf7afb1-4358-4c54-b902-cc209e9b6a52
draw(penguin_bill * mapping(color = :species => "Species"); axis = axis)

# ╔═╡ 47c672d5-7a10-4485-b78c-6feaa4cce475
draw(
	linear() * penguin_bill * mapping(color = :species => "Species");
	axis = (; axis..., title = "Linear Regression")
)

# ╔═╡ fa2dee2b-5aab-47f3-8842-af6d8611f672
draw(
	penguin_bill * mapping(color = :species => "Species") * (
		linear() + mapping(marker = :sex => "Sex")
	);
	axis = axis
)

# ╔═╡ 1bf2588a-c0fa-4e80-bd5c-52c2bdf69d3c
draw(
	penguin_bill * mapping(color = :species => "Species") * (
		linear() + mapping(col = :sex => "Sex")
	);
	axis = axis
)

# ╔═╡ 2fe43c46-1953-485b-8fc6-8146d045ba59
draw(
	penguin_bill * 
	mapping(color = :species => "Species", col = :sex => "Sex") * 
	(
		linear() + mapping()
	);
	axis = axis
...
# ╔═╡ 275c6f09-a576-4723-be3c-3ee004e1927a
draw(
	penguin_bill * density(npoints = 50) * mapping(col = :species);
	axis = axis
)

# ╔═╡ 6fa4b685-cab9-48fb-b94b-66f37f78886f
draw(
	penguin_bill * density(npoints = 50) * mapping(col = :species),
	scales(
		Color = (
...
# ╔═╡ a277bf2b-2831-45f9-bab2-86225eadf759
let
	axis = 
	layer = density() * visual(Wireframe, linewidth=0.05)
	plt = penguin_bill * layer * mapping(color = :species => "Species")
	draw(
		plt;
...
# ╔═╡ 9f4cbbac-0c59-4dd3-b8ee-382c73efde10
let
...
# ╔═╡ f034b465-cd9b-4f4a-9240-06abf8fb473f
let
...
manifest_format = "2.0"
project_hash = "a903006c5fe3753e690015721f521afe07c5045c"
...
git-tree-sha1 = "d61dfac969559522901d03196554a2e4d3d6a76b"
...
git-tree-sha1 = "711256dbb32d0a85ef0ea721d7729f154014c2b2"

...
# ╔═╡ bda73e15-89c4-4194-b573-987045b31b01
using CairoMakie

# ╔═╡ 941fa272-7433-11ef-2f4b-a34c747ab957
using AlgebraOfGraphics

# ╔═╡ af695dcf-e286-41ff-b776-d7e3b835a9e8
let
	x = range(-π, π, length=100)
	y = sin.(x)
	df = (; x, y)
	xy = data(df) * mapping(:y, :x)
	layer = visual(Lines)
	draw(layer * xy)
end

# ╔═╡ b79a56b5-3801-46a3-ae2f-47154ca8b944
let
	x = range(-π, π, length=100)
	y = sin.(x)
	df = (; x, y)
	xy = data(df) * mapping(:y, :x)
	layer = visual(Lines) + visual(Scatter)
	draw(layer * xy)
end

# ╔═╡ b036836e-16c8-4833-9542-7921a668f9fc
let
	x = range(-π, π, length=100)
	y = sin.(x)
	df1 = (; x, y)
	df2 = (x=rand(10), y=rand(10))
	layers = data(df2) * visual(Lines) * mapping(:y, :x) + data(df1) * visual(Scatter) * mapping(:x, :y)
	fg = draw(layers)
end

# ╔═╡ 49aba776-9146-4bb8-aaba-5801e963d713
rand(["a", "b"], 1000)

# ╔═╡ 0c0910d5-cfb2-4b80-902a-6ddaaf54d418
let
	df = (;
...
# ╔═╡ c0277a76-f300-458c-9e32-8f712ef126a4
repeat(["A", "B", "C", "D", "E", "F", "G", "H"], inner = 20)

# ╔═╡ 8ed9b662-350d-47cd-8b9f-07b7d9e3fe6a
@doc pregrouped

# ╔═╡ 9291c921-6a3d-4af6-ad67-11e4a40868a7
df = (
    x = repeat(repeat(1:10, 10), 10),
    y = repeat(repeat(11:20, inner = 10), 10),
    z = randn(1000) .+ repeat(1:10, inner = 100),
    group = repeat(1:10, inner = 100),
)

# ╔═╡ a50f2548-af9e-45a0-8b88-d3925c3c5516
data(df) * mapping(:x, :y, :z, layout = :group => nonnumeric) * visual(Heatmap) |> draw

# ╔═╡ a66ffe7d-d95c-489c-8ceb-cf133b00d5c7
pregrouped(
	fill(1:5, 5),
	fill(11:15, 5),
	[randn(5, 5) .+ i for i in 1:5],
	layout = 1:5 => nonnumeric
) * visual(Heatmap) |> draw

# ╔═╡ 4479911a-ea1c-4250-b599-4c09600b17a8
function generate_random_sound_speed_profile(Nz = rand(10:31))
...
manifest_format = "2.0"
project_hash = "17ecff24b3293463ff8ddfb5daacab45fca97fc1"
